...
pub struct ApiError {
    code: i64,
    message: String,
    errors: Vec<ApiErrorTypes>,
}

#[derive(Deserialize, Debug)]
#[serde(rename_all = "camelCase")]
pub struct ApiErrorTypes {
    error: String,
...
impl CircleClient {
    ...
    async fn send_request<T: DeserializeOwned>(
        ...
        let request = self.client.request(method, &url).bearer_auth(&self.api_key).json(&body);
    ...
    async fn parse_response<T: DeserializeOwned>(response: Response) -> Result<T> {
        let request_id = response.headers().get("X-Request-Id").ok_or(CircleError::MissingRequestId)?;
        let request_id = request_id.to_str()?;
        let request_id = Uuid::parse_str(request_id)?;

        if response.status().is_success() {
            response.json::<ApiSuccess<T>>().await.map(|res| res.data).map_err(From::from)
        } else {
...
mod test {
    ...
    async fn test_parse_wallet_set_response() {
        ...
        let wallet_set_response = serde_json::from_str::<ApiSuccess<CreateWalletSetResponse>>(json).unwrap();
